...
namespace HmsPlugin
{
	...
	{
		private const string PATH = "Assets/Plugins/Android/";

		private const string BOOK_INFO = "BookInfo.java";
		private const string OBJECT_TYPE_INFO_HELPER = "ObjectTypeInfoHelper.java";
		private const string APP_DEBUG = "app-debug.aar";
		...
		{
			var bookInfo = AssetImporter.GetAtPath(PATH + BOOK_INFO) as PluginImporter;
			var objectTypeInfoHelper = AssetImporter.GetAtPath(PATH + OBJECT_TYPE_INFO_HELPER) as PluginImporter;
			var appDebug = AssetImporter.GetAtPath(PATH + APP_DEBUG) as PluginImporter;

			var needBookCompatible = false;
			var needTypeInfoHelperCompatible = false;
			var needAppDebug = false;

#if UNITY_ANDROID && BUILD_HUAWEI
			PrepareGradleFile();

			needBookCompatible = HMSMainEditorSettings.Instance.Settings.GetBool(CloudDBToggleEditor.CloudDBEnabled);
            needTypeInfoHelperCompatible = HMSMainEditorSettings.Instance.Settings.GetBool(CloudDBToggleEditor.CloudDBEnabled);
			needAppDebug = true;
#endif

			if (bookInfo != null)
			{
				bookInfo.SetCompatibleWithPlatform(BuildTarget.Android, needBookCompatible);
				bookInfo.SaveAndReimport();
			}

			if (objectTypeInfoHelper != null)
			{
				objectTypeInfoHelper.SetCompatibleWithPlatform(BuildTarget.Android, needTypeInfoHelperCompatible);
				objectTypeInfoHelper.SaveAndReimport();
			}

			if (appDebug != null)
			{
				appDebug.SetCompatibleWithPlatform(BuildTarget.Android, needAppDebug);
				appDebug.SaveAndReimport();
			}
		}
