...
public class VkSettings: ScriptableObject  {
	...
	public bool stories;		//Доступ к историям.
	public bool plus256;		//Добавление ссылки на приложение в меню слева.
	public bool email;			//Доступ к email пользователя.
	public bool market;			//Доступ к товарам.
	void Awake(){
		generateScope();
	}
	public void generateScope()
	{
		...
		if(stories)scope.Add("stories");
		if(plus256)scope.Add("+256");
		if(email)scope.Add("email");
		if(market)scope.Add("market");
	}
	public void ProcessAuthUrl(){
		if (string.IsNullOrEmpty (PlayerPrefs.GetString("auth_url",""))) {
			...
			if(tokenData.ContainsKey("email")){
				VkApi.CurrentToken.email = tokenData["email"];
			}
