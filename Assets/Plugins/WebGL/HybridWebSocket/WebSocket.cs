...
namespace NativeWebSocket
{
	...
	public class WebSocket : IWebSocket
	{
		...
		[DllImport("__Internal")]
		public static extern string WebSocketGetUrl(int instanceId);

		public int InstanceId { get; private set; }

		public string Url { get { return WebSocketGetUrl(InstanceId); } }
		...
		private async Task WaitForOpen()
		{
	...
	public class WebSocket : IWebSocket
	{
		...
		/* Думал подставить в result.MessageType == WebSocketMessageType.Close
		 * Пока что воздержусь.
		 * Даниил
		 */
		public async Task CloseByMessage(WebSocketCloseCode code = WebSocketCloseCode.Normal, string reason = null)
		{
			if (State == WebSocketState.Connecting || State == WebSocketState.Open)
			{
				/* If you are sending data, and at the same time try to CloseAsync this leads to an exception because CloseAsync will also try to send a message.
				 * The other option is to use CloseOutputAsync this is more of a “fire-and-forget” approach.
		...
		private async Task WaitForSendEnd()
        {
			while (isSending)
				await Task.Yield();
        }
