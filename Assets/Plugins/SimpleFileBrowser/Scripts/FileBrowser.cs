...
#pragma warning restore 0414
			...
			if( allFileEntries != null )
			{
				if( sortFilesByName )
				{
					...
					Array.Sort( allFileEntries, ( entry1, entry2 ) =>
					{
						...
						{
							bool is1Number = int.TryParse(entry1.NameWithoutExtension, out int entry1Num);
							bool is2Number = int.TryParse(entry2.NameWithoutExtension, out int entry2Num);

							if (is1Number && is2Number)
								return entry1Num.CompareTo(entry2Num);
		...
		private int FilenameInputToFileEntryIndex(string input, int startIndex, int length)
		{
			...
			for (int i = 0; i < validFileEntries.Count; i++)
			{
				if (validFileEntries[i].NameWithoutExtension.Length == length && input.IndexOf(validFileEntries[i].NameWithoutExtension, startIndex, length) == startIndex)
					return i;
			}
