...
namespace Assets.Scripts.Utils
{
    public static class TweenUtils 
    {
        ...
        {
            var seq = target.GetScaleFadeAnim(0f.toVector3(), endScale,  0f, 1f,  duration).SetEase(ease);
        ...
        public static Sequence AddAppearAnimation(this CanvasGroup target, float start, float finish,
            ...
            => target.AddAppearAnimation(start.toVector3(), finish.toVector3(), duration, ease, playImmediately);
        ...
        public static IPromise SetAlphaTween(this GameObject o, float alpha, float dur = .25f)
        {
            ...
            var skeletons = o.GetComponentsInChildren<SkeletonGraphic>();

			if (imgs.Length == 0 && txts.Length == 0 && sprites.Length == 0 && skeletons.Length == 0)
            {
                o.SetActive(alpha > 0);
                return Promise.Resolved();
            }
		...
		public static Sequence BounceSequence(this Transform t, float duration, float magnitude = .05f)
		{
			...
			const Ease ease = Ease.InOutBack;
		...
		/// </summary>
		public static Tween Bezier2TweenTransform(this Transform target, Vector3 startPos, Vector3 control, Transform targetTransform, float duration, Ease ease = Ease.Linear)
		{
			float t = 0f;
			void Setter(float x)
			{
				t = x;
				target.position = MathUtils.Bezier2(startPos, control, targetTransform.position, x);
			}
			return DOTween.To(() => t, Setter, 1f, duration).SetEase(ease).SetUpdate(UpdateType.Late).SetLink(target.gameObject);
		}
		...
		public static Tween Bezier2Tween(this Transform target, Vector3 control, Vector3 posEnd, float duration, bool global = false, Ease ease = Ease.Linear) =>
			target.Bezier2Tween(global ? target.position : target.localPosition, control, posEnd, duration, global, ease);
