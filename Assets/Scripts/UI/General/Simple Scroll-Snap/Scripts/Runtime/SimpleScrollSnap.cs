...
namespace DanielLochner.Assets.SimpleScrollSnap
{
	...
	public class SimpleScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
	{
		...
		private void Setup()
		{
			...
			// Panels
			if (sizeControl == SizeControl.Fit) size = new Vector2(GetComponent<RectTransform>().rect.width, GetComponent<RectTransform>().rect.height);
			...
			if (setAsLastSibling)
			{
				for (int i = 0; i < NumberOfPanels; i++)
				{
					PanelsRT[i].SetAsFirstSibling();
				}
			}

			var panelsPositions = new List<Vector2>();
			if (sizeControl == SizeControl.PerElement)
			{
				for (var j = 0; j < PanelsRT.Length; j++)
					panelsPositions.Add(j > 0 ? panelsPositions[j - 1] + PanelsRT[j - 1].rect.size * (automaticLayoutSpacing + 1f) : Vector2.zero);

				for (var j = 0; j < PanelsRT.Length; j++)
					panelsPositions[j] = panelsPositions[j] + PanelsRT[j].rect.size / 2f;
			}

			for (int i = 0; i < NumberOfPanels; i++)
			{
				var curElementSize = sizeControl == SizeControl.PerElement ? PanelsRT[i].rect.size : size;
				...
				if (movementType == MovementType.Fixed && automaticallyLayout)
				{
					...
					Vector2 marginOffset = new Vector2(x / curElementSize.x, y / curElementSize.y);
					PanelsRT[i].pivot = new Vector2(0.5f, 0.5f) + marginOffset;
					PanelsRT[i].sizeDelta = curElementSize - new Vector2(leftMargin + rightMargin, topMargin + bottomMargin);

					float panelPosX = (movementAxis == MovementAxis.Horizontal)
						? sizeControl == SizeControl.PerElement
							? panelsPositions[i].x
			...
			if (movementType == MovementType.Fixed)
			{
				...
				if (automaticallyLayout)
				{
					...
					float contentWidth = (movementAxis == MovementAxis.Horizontal)
						? sizeControl == SizeControl.PerElement
							? PanelsRT.Sum(x => x.rect.width * (automaticLayoutSpacing + 1f))
					...
					float contentHeight = (movementAxis == MovementAxis.Vertical)
						? sizeControl == SizeControl.PerElement
							? PanelsRT.Sum(x => x.rect.height * (automaticLayoutSpacing + 1f))
		...
		public void UpdateButtons()
		{
			if (!infinitelyScroll)
			{
				if (previousButton != null)
				{
					previousButton.gameObject.SetActive(TargetPanel > 0);
				}

				if (nextButton != null)
				{
					nextButton.gameObject.SetActive(TargetPanel < NumberOfPanels - 1);
				}
			}
		}
